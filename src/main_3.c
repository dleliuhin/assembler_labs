    ...
    if (R1 == 15)
    {
        ...
        else
        {
            if (R2 != 0)
            {
                ...
                waddstr(wcyan, "jump to address = 0 or end program tracing after pressing");
                wrefresh(wcyan);
                ret = 1;
            }
        }
...
int P_MVC()
{
    ...
    for (i = 0; i <= L1; i++)
        OBLZ[BAS_IND + CUR_IND + offset1 + i] = OBLZ[BAS_IND + CUR_IND + offset2 + i];

    return 0;
}

void packed_arithmetic(int is_addition)
{
    int i, j;
    int offset1, offset2;
    unsigned char tmp1[56], tmp2[56];
    ...
    for (i = 0; i <= L1 - 1; i++)
    {
        tmp1[i*2]     = (OBLZ[BAS_IND + CUR_IND + offset1 + i] >> 4)   + '0';
        tmp1[i*2 + 1] = (OBLZ[BAS_IND + CUR_IND + offset1 + i] & 0x0F) + '0';
    }
    tmp1[i*2]     = (OBLZ[BAS_IND + CUR_IND + offset1 + i] >> 4) + '0';
    tmp1[i*2 + 1] = '\0';
    ...
    for (i = 0; i <= L2 - 1; i++)
    {
        tmp2[i*2]     = (OBLZ[BAS_IND + CUR_IND + offset2 + i] >> 4)   + '0';
        tmp2[i*2 + 1] = (OBLZ[BAS_IND + CUR_IND + offset2 + i] & 0x0F) + '0';
    }
    tmp2[i*2]     = (OBLZ[BAS_IND + CUR_IND + offset2 + i] >> 4) + '0';
    tmp2[i*2 + 1] = '\0';

    if (is_addition)
        sprintf(tmp1, "%d", atoi(tmp1) + atoi(tmp2));
    else
        sprintf(tmp1, "%d", atoi(tmp1) - atoi(tmp2));

    j = strlen(tmp1) - 1;
    OBLZ[BAS_IND + CUR_IND + offset1 + L1] = ((tmp1[j--] - '0') << 4) + 0x0C;
    for (i = L1 - 1; i >= 0; i--)
    {
        if (j >= 0)
        {
            if (j >= 1)
            {
                OBLZ[BAS_IND + CUR_IND + offset1 + i] = (tmp1[j - 1] - '0') << 4;
                OBLZ[BAS_IND + CUR_IND + offset1 + i] += tmp1[j] - '0';
            }
            else
                OBLZ[BAS_IND + CUR_IND + offset1 + i] = tmp1[j] - '0';
            j -= 2;
            ...
            OBLZ[BAS_IND + CUR_IND + offset1 + i] = 0x00;
    }
}
...
int FSS(int type)
{
    ...
    for (i = 0; i < NOP; i++)
    {
        if (INST[0] == T_MOP[i].CODOP)
        {
            ...
            B1 = INST[2] >> 4;
            D1 = (INST[2] % 16) * 256 + INST[3];
            B2 = INST[4] >> 4;
            D2 = (INST[4] % 16) * 256 + INST[5];
            if (type == 1)
            {
                L1 = INST[1];
                wprintw(wgreen, "X'%.3X'(%d,%d),X'%.3X'(%d)", D1, L1, B1, D2, B2);
            ...
            else
            {
                L1 = INST[1] >> 4;
                L2 = INST[1] & 0x0F;
                wprintw(wgreen, "X'%.3X'(%d,%d),X'%.3X'(%d,%d)", D1, L1, B1, D2, L2, B2);
                wprintw(wgreen, "%.06lX<-%.06lX\n", VR[B1] + D1, VR[B2] + D2);
            }
...
int sys(void)
{
    ...
    waddstr(wmargenta, "\"PgUp\",\"PgDn\",\"Up\",\"Down\"->show dump; \"Enter\"->execute next command");
    ...
    gr_pos_y = 14;
    gr_y = 11;
    wgreen = newwin(gr_y, 70, gr_pos_y, gr_pos_x);        //создадим новое окно
...
l0:
    ...
    waddstr(wcyan, "ready to execute next command with address ");
...
SKIP:
    ...
    {                                              //селектируемой сч.адреса
        ...
        case '\x5B' : P_S();
            break;
        case (unsigned char) '\xD2' : P_MVC();
            break;
        case (unsigned char) '\xD5' : P_AP();
            break;
        case (unsigned char) '\xFB' : P_SP();
            break;
        case '\x47' : P_BC();
            break;
    }
